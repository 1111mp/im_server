...
const { findOne, createOne, updateStatus, getCount } = require('../controllers/star')
const { STARREDISKEY } = require('../const')
...
async function syncStarFromRedisTodb() {
	try {
		...
		starRedisData && await Promise.all(
		...
		logs.conlog({ position: 'syncStarFromRedisTodb', err: JSON.stringify(err) })
	}
...
async function syncStarCountToDb() {
	...
	try {
		...
		starRedisCounts && await Promise.all(
			Object.keys(starRedisCounts).map(async key => {
				...
				const redisCount = Number(starRedisCounts[key] || 0)

				const dbCount = await getCount(dynamicId)

				if (redisCount === dbCount) {
					/** 数据库中的数量的redis中的点赞数量一致 */
					logs.conlog({ msg: '本次同步结果一致', detail: `dynamicId：${key} redis中的点赞数量：${redisCount} db中查询到的数量：${dbCount}` })
				} else {
					logs.conlog({ msg: '本次同步出现错误', detail: `dynamicId：${key} redis中的点赞数量：${redisCount} db中查询到的数量：${dbCount}` })
				}
	...
	logs.conlog({ msg: '每两小时执行一次 执行=======================>>>>>>>>>>>>>>>>>>>>结束' })
}
